...
public class TriFace
{
	...
	public TriFace( Mesh mesh, Vector3[] verts, int[] indis, int resolution)
	{
		this.mesh = mesh;
...
public class IcoSphere : MonoBehaviour
{
	...
	private int lastRecursionLevel = 0;
	...
	private void OnValidate()
	{
		...
		shapeNeedsUpdate = (recursionLevel != lastRecursionLevel);
		if (shapeNeedsUpdate)
		{
			OnShapeUpdated();
			UpdateShapeSettingsValues();
	...
	void Initialize()
	{
		...
		for( int i = 0; i < maxFaces; ++i )
		{
			...
			triFaces[i] = new TriFace(meshFilters[i].sharedMesh, vertices, idx, recursionLevel);
		}
	...
	void CreateSphere()
	{
		foreach( TriFace face in triFaces )
		{
			face.CreateFaces();
		}
	}
	...
	void ComputUpdate()
	{
		...
		for (int i = 0; i < maxFaces; ++i)
		{
			Vector3[] allVerts = triFaces[i].GetVerts();
